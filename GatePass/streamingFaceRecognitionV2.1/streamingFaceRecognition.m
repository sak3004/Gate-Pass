...
if dirExists
	...
	refresh = questdlg(prompt,'Face Recognition Options','START OVER','Add Face(s)','Start Detection','Start Detection');
	refreshOption = find(ismember({'START OVER','Add Face(s)','Start Detection'},refresh));
else
...
while double((get(fdrFig,'currentCharacter')) ~= 27) && ~isDone
	...
	title([bestGuess '?']);
    wp='http://localhost:3000/Page1.html?rollno=';
    u=strcat(wp,bestGuess);
    web(u,'-browser');     
     
%      break;
     
%      detectOption = find(ismember({'Yes','No'},detect));
%      waitfor(detectOption==1||detectOption==2);
     
%      if(detectOption==2) 
%                   break;
%      elseif(detectOption==1)
%          continue;
%      end   
break;
end
...
delete(fdrFig)
% fig=figure('KeyPressFcn',@checkForEnter);
% close(fig);
% waitfor(gcf,'CurrentCharacter');

% curChar=uint8(get(gcf,'CurrentCharacter'));
waitfig=figure('Position',[1250,50,0,0]);
% set(waitfig,
...
% NESTED SUBFUNCTIONS
    ...
    function checkForEnter(varargin)
